  ...
  <div class="modal fade" id="modalCreate" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form id="formStore" method="POST" action="{{ route('category-items.store') }}">
          @csrf
    ...
    <script>
      $(document).ready(function () {
        ...
        let dataTable = $('#Datatable').DataTable({
          ...
          pageLength: 10,
          ajax: {
            url: `{{ route('category-items.index') }}`
          },
          columns: [
            {data: 'name', name: 'name'},
          ...
          rowGroup: {
            startRender: function (rows, group) {
              ...
              return $('<tr style="background-color: #e0e0e0">').append('<td colspan="2">' + group + ' ' + '(' + rows.count() + ')</td>')
            },
            dataSrc: 'kecamatan_nama'
          },
          rowCallback: function (row, data) {
            let api = this.api();
            $(row).find('.btn-delete').click(function () {
              let pk = $(this).data('id'),
                url = `{{ route("category-items.index") }}/` + pk;
              Swal.fire({
                title: "Anda Yakin ?",
        ...
        modalEdit.addEventListener('show.bs.modal', function (event) {
          ...
          this.querySelector('#formUpdate').setAttribute('action', '{{ route("category-items.index") }}/' + event.relatedTarget.getAttribute('data-bs-id'));
        });
